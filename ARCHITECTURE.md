...
- Keep persistence behind typed Supabase clients.
- Keep authentication and identity tied to Supabase Auth.
...
- Supabase client helpers are the database access layer.
...
Server Components may call Supabase-backed query functions, read server-only config, and compose Client Components. They should not include client hooks, event handlers, or browser APIs.
...
- `auth`: Supabase auth helpers and role resolution.
- `db`: typed Supabase clients and transaction helpers when needed.
...
- Integration types: specific to Supabase, Resend, or PostHog boundaries.
...
Services should be testable without rendering UI. They may accept dependencies or run inside Supabase-backed transactions or RPCs when consistency requires it.
...
```text
Route -> Server Component -> feature data query -> Supabase -> view model -> UI
```

Mutation flow:

```text
Form -> Server Action -> auth -> Zod -> service -> Supabase -> side effects -> typed result -> UI state
```

File upload flow:

```text
Client selects file -> server validates intent -> signed upload or controlled upload path -> Supabase Storage -> Supabase metadata -> UI confirmation
```
...
Supabase Auth owns user authentication. EcoLink owns domain profiles and roles. Supabase user IDs should map to internal user profiles in the database.

Typical flow:

1. User signs in through Supabase Auth.
2. Server reads Supabase identity.
...
Legacy database models use PascalCase and camelCase. New Supabase-native tables should prefer snake_case names. Routes use kebab-case. Component files use kebab-case or PascalCase consistently according to the final project convention.
...
- Raw Supabase calls scattered across UI files.
