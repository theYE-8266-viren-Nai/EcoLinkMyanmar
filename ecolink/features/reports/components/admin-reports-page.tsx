...
export function AdminReportsPage({
  ...
  async function loadReports() {
    ...
    const body = (await response.json()) as PendingReportsResponse;
    setRefreshing(false);
    if (!response.ok || "error" in body) {
      setMessage({ kind: "error", text: "error" in body ? body.error : "Could not load pending reports." });
      return;
    }
  ...
  return (
    <main className="admin-page">
      ...
      <div className="admin-container admin-reports-container">
        <div className="admin-title"><div><p>Admin MVP</p><h1>Pending reports</h1><span>Approve or reject community reports before points can be claimed.</span></div><Link className="back-link" href="/admin"><ShieldCheck size={17}/> Staff portal</Link></div>
