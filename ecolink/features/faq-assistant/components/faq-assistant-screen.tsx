  ...
  return (
    ...
    >
      <span className="faq-video-thumb">
        <Image alt="" height={63} src={video.thumbnailUrl} unoptimized width={112} />
    ...
    <main className="content-container faq-page">
      ...
      <section className="faq-chat-shell" aria-label="EcoGuide chat">
        ...
        <form className="faq-composer" onSubmit={(event) => { event.preventDefault(); void send(); }}>
          <label className="sr-only" htmlFor="faq-question">Ask EcoGuide</label>
          <textarea id="faq-question" value={question} maxLength={700} placeholder="Example: should I remove the cap from a plastic bottle?" onChange={(event) => setQuestion(event.target.value)} />
