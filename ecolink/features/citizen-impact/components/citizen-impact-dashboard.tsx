...
export function CitizenImpactDashboard() {
    ...
    <main className="content-container home-page">
      <section className="home-hero">
        <div className="home-hero__content">
          ...
          <div className="hero-actions">
            <Link className="button button--light" href="/recycle"><MapPinned size={18} /> Find a center</Link>
            <Link className="button button--glass" href="/rewards"><Gift size={18} /> Use points</Link>
          </div>
        </div>
        <div className="hero-visual" aria-hidden="true">
          <span className="hero-visual__sun" />
          <span className="hero-visual__city" />
          <span className="hero-visual__river" />
          <span className="hero-visual__badge"><Recycle size={28} /></span>
        </div>
      </section>
      ...
      <section className="impact-overview" aria-label="Your EcoLink impact">
        <article className="points-card">
          ...
          <div className="progress-track" role="progressbar" aria-label="Points milestone progress" aria-valuemin={milestoneStart} aria-valuemax={nextMilestone} aria-valuenow={balance}>
            <span style={{ width: `${Math.max(0, Math.min(100, milestoneProgress))}%` }} />
          </div>
      ...
      <div className="home-columns">
        <section className="history-section" aria-labelledby="history-title">
          ...
          <div className="history-list">
            {state.dropOffs.map((item) => {
              const center = PARTNER_CENTERS.find((entry) => entry.id === item.centerId);

              return (
                <article key={item.id}>
                  <span className="history-material"><Recycle size={19} /></span>
                  <div className="history-main">
                    <strong>{materialName(item.materialSlug)}</strong>
                    <p>{center?.name ?? "Partner center"} &middot; {item.weightKg.toFixed(1)} kg</p>
                    <time><CalendarDays size={13} />{DROP_OFF_DATE_FORMATTER.format(new Date(item.recordedAt))}</time>
                  </div>
        ...
        <aside className="material-impact" aria-labelledby="material-impact-title">
          ...
          <div className="material-bars">
            {topMaterials.map(([slug, weight], index) => (
              <div key={slug}>
                <span><strong>{materialName(slug as never)}</strong><b>{weight.toFixed(1)} kg</b></span>
                <div><i style={{ width: `${Math.max(12, (weight / (topMaterials[0]?.[1] ?? 1)) * 100)}%`, background: ["#087c78", "#2f7a4c", "#0b3558", "#d99a2b"][index % 4] }} /></div>
              </div>
            ))}
          </div>
