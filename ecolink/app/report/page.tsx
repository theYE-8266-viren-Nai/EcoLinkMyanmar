    ...
    <AppShell>
      <main className="content-container report-page">
        ...
        <section className="report-workspace">
          ...
          {step === 2 ? <div className="location-step"><div className="location-map"><iframe title="OpenStreetMap for report location" src="https://www.openstreetmap.org/export/embed.html?bbox=96.075%2C16.745%2C96.205%2C16.91&amp;layer=mapnik" loading="lazy"/></div><label><span>Nearby landmark or coordinates</span><div className="input-with-icon"><MapPin size={18}/><input value={location} onChange={(event) => setLocation(event.target.value)} placeholder="Example: Hledan Market, Insein Road"/></div></label><button className="button button--secondary" type="button" onClick={useLocation}><LocateFixed size={17}/> Use my current location</button><label><span>Additional details <small>(optional)</small></span><textarea value={notes} maxLength={500} onChange={(event) => setNotes(event.target.value)} placeholder="Describe what you can see and any immediate risks."/><small>{notes.length}/500</small></label></div> : null}
