  ...
  return (
    <AppShell>
      <main className="content-container rewards-page">
        ...
        {claimCode ? <div className="modal-backdrop" role="presentation"><section className="claim-modal" role="dialog" aria-modal="true" aria-labelledby="claim-title"><button type="button" className="modal-close" onClick={() => setClaimCode("")} aria-label="Close reward claim"><X size={19}/></button><span><TicketCheck size={30}/></span><p>Reward reserved</p><h2 id="claim-title">Show this code at the partner center</h2><strong>{claimCode}</strong><small>Your points were deducted when the reservation was created.</small><button className="button button--primary" type="button" onClick={() => setClaimCode("")}>Done</button></section></div> : null}
      </main>
    </AppShell>
  );
}
