-- Repair PostGIS operator resolution for get_public_waste_map.
-- The function uses an empty search_path for SECURITY DEFINER safety, so
-- operators from the extensions schema must be explicitly qualified.
