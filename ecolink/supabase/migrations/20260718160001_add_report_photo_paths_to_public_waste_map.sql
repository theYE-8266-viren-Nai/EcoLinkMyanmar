-- Add report photo paths only to close-zoom public map report rows.
-- Aggregated heatmap rows remain count/score only, so broad map views do not
-- expose report-level evidence.
  ...
  if requested_zoom < 12 then
    ...
    select
      ...
      jsonb_build_object(
        ...
        'photoStoragePath', report.photo_storage_path
      )
    from public.environment_reports report
    where report.observed_at >= observed_since
      and report.location OPERATOR(extensions.&&) viewport
